...
export class AurhService {
	...
	async registr(dto: AuthDto) {
		const oldUser = await this.prisma.user.findUnique({
			where: {
				email: dto.email
			}
		})
		if (oldUser) throw new BadRequestException('такой еиамл уже существует')
	...
	private async validateUser(dto: AuthDto) {
		...
		if (!user) throw new NotFoundException('тпользователь не найден')
